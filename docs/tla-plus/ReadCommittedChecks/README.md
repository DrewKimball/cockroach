...
- It refreshes from the RC statement's read timestamp up to the surrounding transaction's provisional commit timestamp. The timestamp cache is bumped accordingly.
...
**Provisional Commit Timestamp (Write Timestamp):**
...
- The final value of the provisional commit timestamp when the transaction commits
- All intents are resolved at this timestamp

### For SSI Transactions

SSI transactions:
1. Read at a fixed transaction-level read timestamp
2. Write at the provisional commit timestamp (which can advance)
3. Before commit, **must** refresh all reads from `read_ts` to `provisional_commit_ts`
4. Commit at the final value of `provisional_commit_ts`

### For RC Transactions (The Proposal)

RC transactions normally:
1. Read at a per-statement read timestamp
2. Write at the provisional commit timestamp
...
2. Refresh check/cascade reads from `stmt_read_ts` to current `provisional_commit_ts`
3. Fails if it encounters any intent
4. Fails if it encounters any committed value written in `(stmt_read_ts, provisional_commit_ts]`
5. On success, updates the timestamp cache to prevent future conflicting writes

### Timestamp Advancement

The spec models two scenarios where the provisional commit timestamp can advance:

1. **Intra-statement advancement** (`stmt_ts_skew`): The provisional commit timestamp advances between the read and write within a single statement (timestamp cache conflict)
2. **Pre-commit advancement** (`commit_ts_skew`): The provisional commit timestamp advances after the statement completes but before the transaction commits
...
3. Allocates provisional_commit_ts=1
4. provisional_commit_ts advances to 2 (timestamp cache conflict)
5. Writes K2 at ts=2 with intent
6. Does NEW per-statement refresh on K1 from stmt_read_ts=1 to provisional_commit_ts=2
7. No writes found in (1, 2], refresh succeeds, bumps tscache[K1]=2
8. Commits with final_commit_ts=2
...
4. TXN2 allocates provisional_commit_ts=3
5. TXN2 writes K2 at ts=3 with intent
6. TXN2 does per-statement refresh on K1 from stmt_read_ts=1 to provisional_commit_ts=3
...
2. Allocates txn read_ts=1, provisional_commit_ts=1
3. Reads K1 at txn read_ts=1, value=0
4. Writes K2 at provisional_commit_ts=1 with intent
5. provisional_commit_ts advances to 2 before commit
6. Does commit-time refresh on K1 from txn read_ts=1 to provisional_commit_ts=2
7. No writes found in (1, 2], refresh succeeds
8. Commits with final_commit_ts=2

### Case 4: SSI transaction aborts when read timestamp cannot be refreshed

1. TXN2 (SSI) begins with txn read_ts=1, provisional_commit_ts=1
2. Reads K1 at txn read_ts=1, value=0
3. (Meanwhile) TXN1 writes K1 at ts=2, commits
4. TXN2 writes K2 at provisional_commit_ts=1 with intent
5. TXN2's provisional_commit_ts advances to 3
6. TXN2 does commit-time refresh from txn read_ts=1 to provisional_commit_ts=3
7. Finds committed value at ts=2 in (1, 3], refresh FAILS
8. TXN2 aborts
...
2. TXN1 allocates provisional_commit_ts=1, writes K1 at ts=1 with intent
3. TXN1 refreshes K2 from stmt_read_ts=1 to provisional_commit_ts=1
4. No conflicts, bumps tscache[K2]=1
5. TXN1 commits with final_commit_ts=1, resolves K1 intent at ts=1
6. TXN2 (RC) allocates stmt_read_ts=2, reads K1 at ts=2
7. Sees TXN1's committed value at ts=1 (OK, ts=1 ≤ stmt_read_ts=2)
8. TXN2 allocates provisional_commit_ts=2, tries to write K2 at ts=2
9. tscache[K2]=1, so write succeeds (ts=2 > tscache=1)
10. TXN2 refreshes K1 from stmt_read_ts=2 to provisional_commit_ts=2
...
1. TXN1 (RC) allocates stmt_read_ts=1, provisional_commit_ts=2
...
8. *[Back to TXN1]*
9. TXN1 commits with final_commit_ts=2
...
4. TXN1 commits (status → "committed", final_commit_ts=2)
...
With `commit_ts_skew=true`, provisional_commit_ts can advance after the statement:

1. TXN1 (RC) allocates stmt_read_ts=1, provisional_commit_ts=2
2. TXN1 reads K2 at ts=1, writes K1 at ts=2 with intent
3. TXN1 does StatementRefresh on K2 from ts=1 to ts=2, succeeds, bumps tscache[K2]=2
4. *[TXN2 interleaves]*
5. TXN2 (RC) allocates stmt_read_ts=3, provisional_commit_ts=4
6. TXN2 tries to read K1, encounters TXN1's intent at ts=2
7. TXN2 tries to push but TXN1.status = "pending", so blocks
8. *[Back to TXN1]*
9. TXN1 MaybeAdvanceBeforeCommit → provisional_commit_ts advances to 5
10. TXN1 commits with final_commit_ts=5
...
### Case 9: RC refresh catches write in the interval after provisional_commit_ts advances

1. TXN1 (RC) allocates stmt_read_ts=1, provisional_commit_ts=2
2. TXN1 reads K2 at ts=1
3. *[TXN2 interleaves]*
4. TXN2 (RC) allocates stmt_read_ts=3, provisional_commit_ts=4
5. TXN2 reads K1 at ts=3, writes K2 at ts=4 with intent
6. TXN2 refreshes K1, commits, resolves K2 intent at ts=4
7. *[Back to TXN1]*
8. TXN1 writes K1 at ts=2 (or later if provisional_commit_ts advanced)
9. TXN1's provisional_commit_ts advances to ts=5 (commit_ts_skew=true)
10. TXN1 does StatementRefresh on K2 from stmt_read_ts=1 to provisional_commit_ts=5
11. Finds committed value at ts=4 in interval (1, 5]
12. TXN1 refresh FAILS, aborts

**Result**: The per-statement refresh correctly detects that K2 was written at ts=4, which is between TXN1's read (ts=1) and its current provisional_commit_ts (ts=5), preventing a stale read.

### Case 10: SSI refresh catches write when provisional_commit_ts advances late

1. TXN1 (SSI) allocates txn read_ts=1, provisional_commit_ts=2
2. TXN1 reads K2 at txn read_ts=1
3. TXN1 writes K1 at provisional_commit_ts=2 with intent
4. *[TXN2 interleaves]*
5. TXN2 writes K2 at ts=3, commits, resolves intent at ts=3
6. *[Back to TXN1]*
7. TXN1's provisional_commit_ts advances to ts=4 before commit
8. TXN1 does CommitRefresh from txn read_ts=1 to provisional_commit_ts=4
...
3. TXN2 (RC) allocates stmt_read_ts=3, provisional_commit_ts=4
...
11. TXN1 tries to write K1 at provisional_commit_ts=2
12. Timestamp cache check: tscache[K1]=4 >= provisional_commit_ts=2
13. TXN1's provisional_commit_ts is bumped to ts=5 (allocate new timestamp above tscache)
14. TXN1 writes K1 at ts=5 with intent (write SUCCEEDS)
15. TXN1 does StatementRefresh on K2 from stmt_read_ts=1 to provisional_commit_ts=5
...
9. TXN1 tries to write K1 at provisional_commit_ts=2
10. Timestamp cache check: tscache[K1]=4 >= provisional_commit_ts=2
11. TXN1's provisional_commit_ts is bumped to ts=5
12. TXN1 writes K1 at ts=5 with intent (write SUCCEEDS)
13. TXN1 does CommitRefresh on K2 from txn read_ts=1 to provisional_commit_ts=5
...
**Key insight**: The timestamp cache mechanism works the same regardless of isolation level. Both RC (per-statement refresh) and SSI (commit-time refresh) abort when their refresh detects a stale read in the interval between read timestamp and (bumped) provisional commit timestamp.
