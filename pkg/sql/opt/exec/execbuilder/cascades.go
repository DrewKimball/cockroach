...
// cascadeTriggerBuilder is a helper that fills in exec.PostQuery metadata; it
...
type cascadeTriggerBuilder struct {
	b              *Builder
...
func makeCascadeTriggerBuilder(
	b *Builder, mutationWithID opt.WithID,
) (*cascadeTriggerBuilder, error) {
	cb := &cascadeTriggerBuilder{b: b}
	if mutationWithID == 0 {
...
func (cb *cascadeTriggerBuilder) setupCascade(cascade *memo.FKCascade) exec.PostQuery {
	return exec.PostQuery{
		...
		PlanFn: func(
			...
			return cb.planPostQuery(
				...
				cascade.Builder, cascade.OldValues, cascade.NewValues, 0 /* canaryCol */, actionName,
			)
		},
	}
}
...
func (cb *cascadeTriggerBuilder) setupTriggers(triggers *memo.AfterTriggers) exec.PostQuery {
	return exec.PostQuery{
		...
		PlanFn: func(
			...
			return cb.planPostQuery(
				ctx, semaCtx, evalCtx, execFactory, bufferRef, numBufferedRows, allowAutoCommit,
				triggers.Builder, triggers.OldValues, triggers.NewValues, triggers.CanaryCol, actionName,
			)
		},
	}
}
...
//
// See the comment for cascadeTriggerBuilder for a detailed explanation of the
// process.
func (cb *cascadeTriggerBuilder) planPostQuery(
	ctx context.Context,
	semaCtx *tree.SemaContext,
	evalCtx *eval.Context,
	execFactory exec.Factory,
	...
	if bufferRef == nil {
		...
		*bindingProps.Statistics() = props.Statistics{
		...
		// Remap the cascade/trigger columns.
		oldVals, err = remapColumns(oldVals, withColRemap)
		if err != nil {
			return nil, err
		}
		newVals, err = remapColumns(newVals, withColRemap)
		if err != nil {
			return nil, err
		}
		if canaryCol != 0 {
			canaryCol, err = remapColumn(canaryCol, withColRemap)
			if err != nil {
				return nil, err
			}
		}
...
func remapColumns(cols opt.ColList, m opt.ColMap) (_ opt.ColList, err error) {
	res := make(opt.ColList, len(cols))
	for i := range cols {
		res[i], err = remapColumn(cols[i], m)
		if err != nil {
			return nil, err
		}
	}
...
func remapColumn(col opt.ColumnID, m opt.ColMap) (opt.ColumnID, error) {
	val, ok := m.Get(int(col))
	if !ok {
		return 0, errors.AssertionFailedf("column %d not in mapping %s\n", col, m.String())
	}
	return opt.ColumnID(val), nil
}
