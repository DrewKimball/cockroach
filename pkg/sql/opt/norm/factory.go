...
func (f *Factory) DuplicateSubtree(e opt.Expr) opt.Expr {
	...
	// Disable normalization during duplication to avoid triggering normalization
	// rules that may try to access properties of partially-constructed expressions.
	// Duplication creates a structural copy with remapped IDs, so normalization
	// isn't needed and can cause errors when rules try to access properties that
	// aren't available yet.
	var result opt.Expr
	f.DisableOptimizationsTemporarily(func() {
		result = d.duplicateExpr(e)
	})
	return result
}
