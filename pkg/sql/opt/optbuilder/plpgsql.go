...
func (b *plpgsqlBuilder) buildRootBlock(astBlock *ast.Block, s *scope) *scope {
	...
	for _, outParam := range b.outParams {
		...
		outParamTyp, _ := b.resolveVariableForAssign(outParam)
	...
	for i, stmt := range stmts {
		...
		case *ast.Fetch:
			...
			if !t.IsMove {
				...
				if b.targetIsCompositeVar(t.Target) {
		...
		case *ast.Call:
			...
			for _, targetName := range target {
				typ, _ := b.resolveVariableForAssign(targetName)
...
func (b *plpgsqlBuilder) addPLpgSQLAssign(
	...
	col := b.ob.synthesizeColumn(assignScope, colName, resolvedType, nil, scalar)
	col.setParamOrd(ord)
...
// variable is the target of an INTO clause. In this case, the columns from the
// SQL statement should be wrapped into a tuple, which is assigned to the
...
func (b *plpgsqlBuilder) makeContinuationDef(conName string) *memo.RoutineDefinition {
	return &memo.RoutineDefinition{
...
func (b *plpgsqlBuilder) makeContinuation(conName string) continuation {
	...
	def.Body = &memo.DefaultRoutineBody{Params: b.buildContinuationParams(s)}
	return continuation{def: def, typ: continuationDefault, s: s}
}
...
func (b *plpgsqlBuilder) appendBodyStmtFromScope(con *continuation, bodyScope *scope) {
	...
	body, ok := con.def.Body.(*memo.DefaultRoutineBody)
	if !ok {
		panic(errors.AssertionFailedf("expected a default routine body"))
	}
	body.Body = append(body.Body, bodyExpr)
	body.BodyProps = append(body.BodyProps, bodyScope.makePhysicalProps())
}
...
func (b *plpgsqlBuilder) checkMultipleRecordTargets(target []ast.Variable) {
	...
	for _, varName := range target {
		typ, _ := b.resolveVariableForAssign(varName)
		if typ.Identical(types.AnyTuple) {
...
type continuation struct {
	...
	// paramCount is the number of parameters that the continuation routine
	// takes. This is used to determine which variables are in scope for the
	// continuation.
	paramCount int

	// s is a scope initialized with the parameters of the routine. It should be
	// used to construct the routine body statements. It is not set for lazy
	// continuations.
	s *scope
}
