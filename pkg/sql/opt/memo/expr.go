...
type PostQueryBuilder interface {
	...
	// The input is scanned using WithScan with the given WithID; oldValues and
	// newValues columns correspond 1-to-1 to foreign key columns. For deletes,
	// newValues is empty. The canaryCol ID is only nonzero for AFTER triggers
	// fired on UPSERT or ON CONFLICT statements.
	//
	...
	// (oldValues/newValues, canaryCol) are valid in the metadata.
	//
	...
	// Triggers always require an input binding.
	//
